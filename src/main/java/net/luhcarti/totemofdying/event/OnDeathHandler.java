...
public class OnDeathHandler implements ServerLivingEntityEvents.AllowDeath {
    ...
    public boolean allowDeath(LivingEntity entity, DamageSource damageSource, float damageAmount) {
        if (entity instanceof PlayerEntity player) {
            ...
            if (player.getMainHandStack().isOf(totem.getItem()) || player.getOffHandStack().isOf(totem.getItem())) {
                ...
                if (!world.isClient) {
                    if (player.getMainHandStack().isOf(totem.getItem())) {
                        player.setStackInHand(Hand.MAIN_HAND, ItemStack.EMPTY);
                    } else if (player.getOffHandStack().isOf(totem.getItem())) {
                        player.setStackInHand(Hand.OFF_HAND, ItemStack.EMPTY);
                    }

                    //This is for the animation, just sends the TotemActivationPacket to all connected clients
                    ClientPlayNetworking.send(ModNetwork.ACTIVATE_TOTEM_ID, PacketByteBufs.create());
